...
pub fn run() {
    ...
    const RADIX: u32 = 10;
    let pixels: Vec<u32> = input[0]
        .chars()
        .map(|c| c.to_digit(RADIX).unwrap() as u32)
        .collect();
    ...
    pixels
        ...
        .for_each(|(l, layer)| {
            ...
            layerhist.iter().enumerate().for_each(|(i, h)| {
                histograms[l][i] = *h;
            });
        });
    ...
    histograms.iter().for_each(|h| {
        if h[0] < least_zeros[0] {
            least_zeros = h.to_vec();
        }
    });
