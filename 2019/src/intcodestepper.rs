...
impl fmt::Display for LoopStatus {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            LoopStatus::Success => write!(f, "Success"),
            // LoopStatus::Halted => write!(f, "Halted"),
            LoopStatus::HaltedForInput => write!(f, "HaltedForInput"),
            LoopStatus::HaltedForOutput => write!(f, "HaltedForOutput"),
...
impl IntCodeStepper {
    pub fn new(m: &BTreeMap<i64, i64>) -> IntCodeStepper {
        IntCodeStepper {
            memory: m.clone(),
            pc: 0,
            relative_base: 0,
    ...
    pub fn has_output(&self) -> usize {
        self.output.len()
    }

    pub fn step(&mut self) -> LoopStatus {
        let mut returnvalue = LoopStatus::Success;
        let mut instructioncode: i64 = self.get_mem(self.pc + 0, Mode::IMM);
        let opcode = instructioncode % 100;

        if opcode == 3 && self.input.len() == 0 {
            return LoopStatus::HaltedForInput;
        }
        ...
        match opcode {
            ...
            4 => {
                ...
                returnvalue = LoopStatus::HaltedForOutput;
            }
            5 => {
                paramsize = 2;
            ...
            99 => {
                paramsize = 0;
                returnvalue = LoopStatus::Success;
            }
            _ => {
                paramsize = 0;
                returnvalue = LoopStatus::Error;
            }
        }
