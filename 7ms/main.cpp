...
int main() {
    ...
    while (parents.size()>0) {
                            ...
                            for (auto c : childless) {
                                if (c.name.compare(*n_iter)==0) {
                                    if (c.size==s.first) {
                                        ...
                                        correct = false;
                                        std::cout << "Found node with error: " << c.name << " with weight " << c.size << std::endl;
                                    }
                                }
                            }
                            ++n_iter;
            ...
            if (!correct) {
                for (auto s : sizes) {
                    if (s.first!=wrong_size) {
                        ...
                        std::cout << "Needs to be adjusted by " << delta << " - should be: " << wrong_orig_size+delta << std::endl;
                        return 0;
                        break;
        ...
        for (auto& p : parents) {
            ...
            for (auto& c : childless) {
                ...
                //std::cout << "parent " << p.name << " has " << p.children.size() << " children" << std::endl;
            }
        }
        ...
        while (p_iter!=parents.end()) {
            //std::cout << p_iter->name << " has size " << p_iter->children.size() << std::endl;
            if (p_iter->children.size()==0) {
                //std::cout << "Moving " << p_iter->name << " to childless." << std::endl;
    ...
    std::cout << "Name of root: " << childless[0].name << " with size " << childless[0].size << std::endl;
}