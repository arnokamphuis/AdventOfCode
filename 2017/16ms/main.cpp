    ...
    switch(cmd[0]) {
        case 's': {
            for (int i=0;i<std::stoi(cmd.substr(1));++i) {
                char c = formation.back(); formation.pop_back();
...
void reset(std::string& formation) {
    for (int i=0;i<formation.size();++i) formation.at(i)='a'+i;   
}

int main() {
    int size=16;

    std::string formation; formation.resize(size,'a');
    reset(formation);
    ...
    std::string cmd;
    while (getline(std::cin,cmd,',')) thedance.push_back(cmd);
    ...
    for (int i=0;i<maxruns;++i) {
        ...
        if (i==0) std::cout << "Part 1: " << formation << std::endl;

        if ( (p=std::find(previous_formations.begin(),previous_formations.end(),formation))==previous_formations.end()) {
            previous_formations.push_back(formation);
        } else { // cycle detected
    ...
    if (cycle_length>0) {
        ...
        for (int i=0;i<runstogo;++i) for(auto dm: thedance) do_move(formation,dm);
    }
