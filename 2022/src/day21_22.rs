...
fn add_node(name: Vec<char>, l: BTNode<i64>, r: BTNode<i64>) -> BTNode<i64> {
    BTNode::new(name.clone(), Op::Add, l, r)
}

fn sub_node(name: Vec<char>, l: BTNode<i64>, r: BTNode<i64>) -> BTNode<i64> {
    BTNode::new(name.clone(), Op::Sub, l, r)
}

fn div_node(name: Vec<char>, l: BTNode<i64>, r: BTNode<i64>) -> BTNode<i64> {
    BTNode::new(name.clone(), Op::Div, l, r)
}

fn mul_node(name: Vec<char>, l: BTNode<i64>, r: BTNode<i64>) -> BTNode<i64> {
    BTNode::new(name.clone(), Op::Mul, l, r)
}

fn id_node(name: Vec<char>, value: i64) -> BTNode<i64> {
    BTNode {
        name: name.clone(),
        op: Op::Id(value), 
        left: None, 
...
impl BinaryTree<i64> {
    ...
    pub fn find_needed_value<'a>(node: &'a Box<BTNode<i64>>, value: i64) -> i64 {
        ...
        if let Some(_) = BinaryTree::find_child(left, &vec!['h','u','m','n']) {
        ...
        if let Some(_) = BinaryTree::find_child(right, &vec!['h','u','m','n']) {
...
impl MonkeyBusiness {
    fn run(&mut self) -> i64 {
        ...
        while self.to_be_done.len() > 0 {
            ...
            let doable = tbd.iter().filter(|((n1,n2),_)| {
    ...
    fn create_node(&self, name: &Vec<char>) -> BTNode<i64> {
        ...
        if item.len() == 1 {
            match item[0].1.1 {
                Operation::Minus  => sub_node( item[0].1.0.clone(), self.create_node(&item[0].0.0), self.create_node(&item[0].0.1) ),
                Operation::Divide => div_node( item[0].1.0.clone(), self.create_node(&item[0].0.0), self.create_node(&item[0].0.1) ),
                Operation::Plus   => add_node( item[0].1.0.clone(), self.create_node(&item[0].0.0), self.create_node(&item[0].0.1) ),
                Operation::Times  => mul_node( item[0].1.0.clone(), self.create_node(&item[0].0.0), self.create_node(&item[0].0.1) ),
            }
        } else {
            id_node(name.clone(), self.yelled[name])
        }
    }
